...
import {
  ...
  CalendarDays, User, TrendingUp, ArrowRight,
} from "lucide-react";
...
const DashboardOperacional = () => {
  ...
  const ongoing = projectsWithMeta.filter((p) => p.normalized === "ongoing");
  const planning = projectsWithMeta.filter((p) => p.normalized === "planning");
  const completed = projectsWithMeta.filter((p) => p.normalized === "completed");
  const lateProjects = ongoing.filter((p) => p.isLate);

  const avgProgressOngoing =
    ongoing.length > 0
      ? Math.round(
          ongoing.reduce((sum, p) => sum + p.avgProgress, 0) / ongoing.length
        )
      : 0;
        ...
        <Card className="p-6 border-destructive/30 bg-destructive/5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {lateProjects.map((p, i) => (
              <div
                key={i}
                className="bg-background rounded-xl p-4 border border-destructive/20 flex flex-col gap-2"
              >
                <div>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {planning.map((p, i) => (
              <div
                key={i}
                className="rounded-xl border border-border bg-muted/30 p-4 flex flex-col gap-1.5 hover:bg-muted/60 transition-colors"
              >
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {upcomingDeadlines.map((p, i) => (
              ...
              >
                ...
                <div className={cn(
                  "text-xs font-bold",
                  (p.daysToEnd ?? 99) <= 7 ? "text-warning" : "text-success"
                )}>
                  {p.daysToEnd === 0
                    ? "Vence hoje!"
                    : `${p.daysToEnd} dia${p.daysToEnd !== 1 ? "s" : ""} restantes`}
                </div>
